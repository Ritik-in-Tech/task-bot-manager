...
const AnalyticsPage: React.FC = () => {
	...
	const botStatusCounts = bots.reduce(
		(acc, b) => {
	...
	const statusPieData = Object.entries(botStatusCounts).map(([key, val]) => ({
		name: key,
		value: val,
	}));
	...
	const priorityBarData = priorities.map((p) => ({
		priority: p,
		...
		<MainLayout>
			...
			<div className="flex flex-col lg:flex-row gap-10 justify-center items-center w-full">
				<div className="bg-card rounded-xl p-6 shadow w-full max-w-sm flex flex-col items-center">
					<span className="mb-2 font-semibold text-md text-center">
								...
								>
									{statusPieData.map((entry) => (
										<Cell
											...
											fill={
												BOT_STATUS_COLORS[
													entry.name as keyof typeof BOT_STATUS_COLORS
												] || "#aaa"
											}
										/>
									))}

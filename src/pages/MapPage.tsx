...
const MapPage: React.FC = () => {
  ...
  }, [bots.length]); // Re-init if bot count changes
      ...
      <div className="flex flex-col items-center gap-6">
        {!svgContent && (
          <div className="flex flex-col items-center gap-4 p-10 border-2 border-dashed rounded-xl bg-muted/20">
            <p className="text-muted-foreground">Upload your warehouse layout (SVG) or use a demo map</p>
        ...
        {svgContent && svgContent !== 'DEMO_3D' && (
          <div className="relative w-full max-w-4xl aspect-video border rounded-xl overflow-hidden bg-white shadow-lg" ref={containerRef}>
            ...
            {botPositions.map((pos) => {
              ...
              const statusColor = 
                bot?.status === 'busy' ? 'bg-orange-500' :
                bot?.status === 'error' ? 'bg-red-500' :
                bot?.status === 'charging' ? 'bg-green-500' : 'bg-blue-500';
        ...
        <div className="flex gap-4 text-sm text-muted-foreground">
            <div className="flex items-center gap-1"><span className="w-3 h-3 rounded-full bg-blue-500"></span> Idle</div>
            <div className="flex items-center gap-1"><span className="w-3 h-3 rounded-full bg-orange-500"></span> Busy</div>
            <div className="flex items-center gap-1"><span className="w-3 h-3 rounded-full bg-green-500"></span> Charging</div>
            <div className="flex items-center gap-1"><span className="w-3 h-3 rounded-full bg-red-500"></span> Error</div>
        </div>
      </div>
    </MainLayout>
  );
